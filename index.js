...
const _ipTokens = {};
// 每次进服IP验证标记 { xuid: true }，玩家离线时清理
var _ipVerifiedThisSession = {};
...
function onJoinHandler(player) {
	...
	if (trueipCfg.enabled && trueipCfg.url && getPlayerSetting(xuid, "enableIpVerify")) {
		// 使用内存标记，每次进服最多弹一次
		if (!_ipVerifiedThisSession) _ipVerifiedThisSession = {};
		if (!_ipVerifiedThisSession[playerXUID]) {
		try {
		...
		} // end _ipVerifiedThisSession check
	}
...
function onLeftHandler(player) {
	...
	delete _ipVerifiedThisSession[xuid];
