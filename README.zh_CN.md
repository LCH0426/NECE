...
**NLCE**（代号：*Robin*）是一款专为 **LegacyScriptEngine-server-nodejs** 设计的功能全面的多功能插件。使用 **JavaScript** 编写，提供从经济商店到 Web 管理面板等一系列丰富功能。

## 功能特性

### 🏪 经济与交易
- **商店系统** — 分类商品浏览与搜索，支持购买与回收
- **经济系统** — 完整的经济体系，包含余额查询与转账功能
- **银行系统** — 存取款、定期存款及利息计算
- **CDK 兑换** — 兑换码兑换奖励，支持使用次数追踪与限制
- **星尘商店** — 使用星尘货币购买物品
- **经验商店** — 使用货币兑换经验值
- **附魔书商店** — 使用星尘购买附魔书
- **刷怪蛋商店** — 使用星尘购买刷怪蛋
...
### 🎮 游戏玩法
- **祈愿系统** — 类似抽卡的祈愿系统，可配置卡池、概率与保底机制
- **VIP 系统** — 限时/永久 VIP，享受商店折扣与专属特权
- **冒险等级系统** — 通过获取经验升级，领取等级奖励
- **属性提升** — 使用星核提升玩家属性（如最大生命值）
- **死亡点返回** — 传送回死亡地点
- **击杀特效** — PvP 击杀时触发视觉特效与增益效果
- **图腾自动替换** — 图腾耗尽后自动从背包替换
- **自定义生命值** — 可配置的最大生命值，支持升级加成

### 🤝 社交互动
- **好友系统** — 发送、接受和管理好友请求，查看好友详情
- **消息系统** — 好友与陌生人私信，对话历史，消息通知（已整合到好友系统）
- **邮件系统** — 完整的邮件系统，支持全体邮件、个人邮件和定时邮件（管理员）
- **留言板** — 社区留言板，支持发帖、回复与管理

### 🌍 传送系统
- **家园系统** — 设置、删除和传送到个人家园点，支持数量限制和冷却时间
- **公共传送点** — 由管理员管理的公共传送点，方便社区快速出行
- **互传系统** — 玩家间传送请求（tpa/tpn/tpy），支持接受/拒绝/取消，冷却时间和可选费用
- **随机传送 (RTP)** — 在可配置范围内随机传送，含冷却时间和出生点保护

### ⚙️ 服务器管理
- **Web 管理面板** — 基于 Express.js 的功能丰富网页端管理后台，使用 JWT 认证，前端修改自 [shadcn-vue-admin](https://github.com/Whbbit1999/shadcn-vue-admin)
- **封禁系统** — 支持按玩家ID/UID/XUID封禁，IP关联封禁，控制台与游戏内管理
- **排行榜** — 经济、存款、击杀、死亡、挖掘等多维度排行
- **NPC 攻击响应 (NAR)** — 配置玩家与 NPC 交互/攻击时的自定义行为
- **聊天系统** — 自定义聊天格式，支持敏感词过滤与公会标签
- **快捷菜单** — 可自定义的快捷访问菜单，支持物品快捷方式

### 📊 监控与日志
- **TPS/MSPT 监控** — 实时服务器 TPS 与 MSPT 追踪
- **侧边栏显示** — 动作栏侧边栏展示余额、延迟、TPS、生物群系、时间等
- **行为日志** — 全面记录玩家行为（加入、方块交互、战斗等）
- **聊天日志** — 完整的聊天历史记录，支持按日期查询
- **管理日志** — 网页端管理员操作的审计追踪
- **网络信息** — 显示 IPv4/IPv6 连接信息

### 🎨 个性化
- **玩家设置** — 每位玩家可独立开关偏好（欢迎消息、侧边栏信息、通知等）
- **头像系统** — 通过 QQ、自定义链接或 Citlalia 头像码设置玩家头像
- **UID 显示** — 在动作栏显示玩家 UID
- **IP 检测器** — 检测并通知玩家的 IPv4/IPv6 连接
...
- [jsonwebtoken](https://www.npmjs.com/package/jsonwebtoken) — JWT 身份认证
- [sql.js](https://www.npmjs.com/package/sql.js) — SQLite 数据库（内存模式）
- [cors](https://www.npmjs.com/package/cors) — 跨域资源共享
- [svg-captcha](https://www.npmjs.com/package/svg-captcha) — 验证码生成
- [csv-parser](https://www.npmjs.com/package/csv-parser) — CSV 日志解析
- [systeminformation](https://www.npmjs.com/package/systeminformation) — 系统监控
- [7zip-min](https://www.npmjs.com/package/7zip-min) — 7zip 压缩（用于备份）
...
本程序为自由软件：您可以依据 **GNU 通用公共许可证第三版 (GPL-3.0)** 的条款对其进行再发布和/或修改，该许可证由自由软件基金会发布，您可以选择适用该许可证的第三版或（根据您的选择）任何后续版本。

本程序的发布旨在使其有用，但**不提供任何担保**；甚至不包含对**适销性**或**特定用途适用性**的暗示担保。详情请参阅 [GNU 通用公共许可证](https://www.gnu.org/licenses/gpl-3.0.en.html)。

您应当已随本程序收到 GNU 通用公共许可证的副本。如未收到，请参阅 <https://www.gnu.org/licenses/>。
